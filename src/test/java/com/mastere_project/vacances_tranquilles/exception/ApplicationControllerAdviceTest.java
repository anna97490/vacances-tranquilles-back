...
class ApplicationControllerAdviceTest {
    ...
    public ResponseEntity<ErrorEntity> handleAll(Exception ex) {
        ErrorEntity error = new ErrorEntity("INTERNAL_ERROR", ex.getMessage());
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(error);
    }

    @ExceptionHandler(org.springframework.web.servlet.NoHandlerFoundException.class)
    public ResponseEntity<ErrorEntity> handleNotFound(Exception ex) {
        ErrorEntity error = new ErrorEntity("NOT_FOUND", "Ressource non trouvée");
